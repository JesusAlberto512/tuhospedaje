...
use DateInterval, DatePeriod, Common;
use Carbon\Carbon;
...
class ReportsController extends Controller
{
    ...
    public function salesAnalysis()
    {
        ...
        for ($i=1; $i <count($monthYears); $i++) {
            ...
            if (!is_numeric($reservationRates[$i]) || !is_numeric($reservationRates[$i-1])){

                if (!is_numeric($reservationRates[$i]) && !is_numeric($reservationRates[$i-1])) {
                    $reservationRateDiff[$i]     = number_format(((str_replace(',', '', $reservationRates[$i])) - (str_replace(',', '', $reservationRates[$i-1]))), 2, '.', ',');
                } elseif (!is_numeric($reservationRates[$i])) {
                    $reservationRateDiff[$i]     = number_format(((str_replace(',', '', $reservationRates[$i])) - $reservationRates[$i-1]), 2, '.', ',');
                } else {
                    $reservationRateDiff[$i]     = number_format(($reservationRates[$i] - (str_replace(',', '', $reservationRates[$i-1]))), 2, '.', ',');
                }
